...
const LendingPoolAddressesProvider = require('../utils/abis/aave/LendingPoolAddressesProvider.json');
const LendingPool = require('../utils/abis/aave/LendingPool.json');
const LendingPoolCore = require('../utils/abis/aave/LendingPoolCore.json');
const LendingPoolDataProvider = require('../utils/abis/aave/LendingPoolDataProvider.json');
const AToken = require('../utils/abis/aave/AToken.json');
...
async function execute(network, action, ...params) {
  ...
  const lendingPoolCore = new eth.Contract(
    LendingPoolCore,
  ...
  if (action == 'repay') {
    ...
    const user = params[1];
    const amount =
      params[2] === 'all' ? maxUint256 : web3.utils.toWei(params[2]);
    let value = 0;
    if (params[0] === 'celo') {
      value = amount;
      if (amount === maxUint256) {
        const reserveData = await lendingPool.methods
          .getUserReserveData(reserve, user)
          .call();
        value = BN(reserveData.currentBorrowBalance)
          .multipliedBy('1.001')
    ...
    try {
      await retry(() =>
        lendingPool.methods
          .repay(reserve, amount, user)
          .estimateGas({ from: user, gas: 2000000, value })
      );
    } catch (err) {
      console.log(
        'Revoke approve',
    ...
    console.log(
      ...
      (
        await lendingPool.methods
          .repay(reserve, amount, user)
          .send({ from: user, gas: 2000000, value })
      ).transactionHash
    );
  ...
  if (action == 'redeem') {
    ...
    try {
      await retry(() =>
        mtoken.methods.redeem(amount).estimateGas({ from: user, gas: 2000000 })
      );
    } catch (err) {
    ...
    console.log(
      'Redeem',
      (await mtoken.methods.redeem(amount).send({ from: user, gas: 2000000 }))
        .transactionHash
    );
    return;
  }
